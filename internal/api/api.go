...
func (api *API) PatchParticipantsParticipantIDConfirm(w http.ResponseWriter, r *http.Request, participantID string) *spec.Response {
	...
	if err != nil {
		...
		return spec.PatchParticipantsParticipantIDConfirmJSON400Response(spec.Error{
			Message: "Algo deu errado, pode tentar novamente?",
		})
	}
	...
	if err := api.store.ConfirmParticipant(r.Context(), id); err != nil {
		api.logger.Error("Erro ao confirmar o participante.", zap.Error(err), zap.String("participant_id", participantID))
		return spec.PatchParticipantsParticipantIDConfirmJSON400Response(spec.Error{
			Message: "Algo deu errado, pode tentar novamente?",
		})
	}
...
func (api *API) PostTrips(w http.ResponseWriter, r *http.Request) *spec.Response {
	...
	if err != nil {
		spec.PostTripsJSON400Response(spec.Error{Message: "JSON inválido: " + err.Error()})
	}
	...
	if err != nil {
		return spec.PostTripsJSON400Response(spec.Error{Message: "Falha ao criar a viagem, pode tentar novamente?"})
	}
